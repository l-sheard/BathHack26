  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden py-12">
      ...
      <div className="relative mx-auto max-w-2xl space-y-6">
        <Card className="space-y-4 bg-white/15 backdrop-blur-md shadow-lg text-black rounded-3xl">
            ...
            <div className="flex flex-wrap gap-2">
              {participantId ? (
                <Button
                  variant="ghost"
                  onClick={() =>
                    navigate(`/trip/${tripId}/preferences/${participantId}`)
                  }
                >
              ...
              {options.data && options.data.length > 0 ? (
                ...
                (() => {
                  ...
                  const hasPreferences = (participants.data ?? []).some(
                    (p: any) =>
                      p.participant_preferences && p.participant_preferences.id,
                  );
                  return (
                    <>
                      <Button
                        ...
                        title={
                          !hasPreferences
                            ? "At least one participant must fill out the trip preference form before generating options."
                            : undefined
                        }
                      >
                      ...
                      {!hasPreferences && (
                        <div className="mt-2 text-xs text-red-600">
                          At least one participant must fill out the trip
                          preference form before generating trip options.
                        </div>
                      )}
                    </>
                  );
            ...
            {(participants.data?.length ?? 0) === 0 ? (
              ...
              <div className="grid gap-2 md:grid-cols-2">
                {(participants.data ?? []).map((participant: any) => {
                  ...
                  const hasVoted = (votes.data ?? []).some(
                    (vote: any) => vote.participant_id === participant.id,
                  );
                  return (
                    <div
                      key={participant.id}
                    ...
                    >
                      ...
                      <div
                        className={`mt-2 text-xs font-semibold ${hasVoted ? "text-green-700" : "text-red-600"}`}
                      >
        ...
        <Modal open={isShareOpen} onClose={() => setIsShareOpen(false)}>
          ...
          <p className="mt-2 break-all rounded-lg bg-slate-100 p-2 text-sm text-black">
