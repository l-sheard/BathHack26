    ...
    <div className="relative isolate grid gap-8 md:grid-cols-[1.2fr_1fr] md:items-center">
      ...
      <section className="relative z-10 space-y-5">
        ...
        <h1 className="font-display text-4xl font-bold leading-tight md:text-6xl text-blue-600">
          For the Plot
          <br />
          Get the group trip out the group chat
        </h1>
        <p className="max-w-xl text-black">
          Create a trip, collect preferences, generate 3 realistic options, vote
          together, and track booking progress in one place.
        </p>
      </section>
      <Card className="relative z-10 space-y-3 border-violet-400/35 shadow-[0_25px_60px_rgba(124,58,237,0.35)] text-black rounded-3xl">
        ...
        <Button
          className="w-full bg-violet-600 hover:bg-violet-700 text-white rounded-full transition-colors duration-200 border-none shadow-md"
        ...
        <div className="space-y-2 border-t border-violet-400/25 pt-3">
          ...
          <Button
            ...
            onClick={() =>
              navigate(
                `/trip/${joinTripId.trim()}/join?code=${encodeURIComponent(joinCode.trim())}`,
              )
            }
